'use client'

import { UserButton, useUser } from '@clerk/nextjs'
import type { Channel, Dm } from '@prisma/client'
import { useQuery } from '@tanstack/react-query'
import type { UseUserReturn } from '@clerk/types'
import { useContext, useEffect } from 'react'
import { CurrentChannelContext } from '@/app/(mainapp)/channels/context'
import Link from 'next/link'
import { usePathname } from 'next/navigation'

const SidebarTopAndMain = ({ children }: { children: React.ReactNode }) => {
  const state = useContext(CurrentChannelContext)
  const user = useUser()
  return (
    <div className='flex flex-col w-full h-full'>
      <Top user={user} />
      <div className='flex flex-row w-full h-full'>
        <ChannelsSidebar user={user} />
        <Main>{children}</Main>
      </div>
    </div>
  )
}

const ChannelsSidebar = ({ user }: { user: UseUserReturn }) => {
  const state = useContext(CurrentChannelContext)
  const dbUser = useQuery({
    queryKey: ['get-db-user'],
    queryFn: async () => {
      const userReq = await fetch('http://localhost:9999/get-user', {
        method: 'POST',
        body: JSON.stringify({ clerk_id: user.user?.id })
      })
      return await userReq.json()
    }
  })
  const channels =
    state.value.type == 'server'
      ? useQuery({
          queryKey: ['get-server-channels'],
          queryFn: async () => {
            const req = await fetch('http://localhost:9999/get-channel', {
              method: 'POST',
              body: JSON.stringify({ serverId: state.value.id?.split('/')[0] })
            })
            return await req.json()
          }
        })
      : (state.value.type == 'dm' ? useQuery({
        queryKey: ['get-dm'],
        queryFn: async () => {
          const req = await fetch('http://localhost:9999/get-dm', {
            method: 'POST',
            body: JSON.stringify({ id: state.value.id }),
            headers: {
              'reis': '1'
            }
          })
          return await req.json()
        }
      }) : useQuery({
        queryKey: ['get-dms-with-user'],
        queryFn: async () => {
          const dmsReq = await fetch('http://localhost:9999/get-dm', {
            method: 'POST',
            body: JSON.stringify({ userId: dbUser.data.id })
          })
          const dms = await dmsReq.json()
          return dms
        }
      }))
  const pathName = usePathname()
  const pathId = pathName.replace('/channels', '')
  useEffect(() => {
    let pathIdType: 'dm' | 'home' | 'server' | null;
    console.log(pathId)
    if (pathId === ''){
      pathIdType = 'home'
    } else if (pathId.includes('/')) {
      pathIdType = 'server'
    } else {
      pathIdType = 'dm'
    }
    state.setter({ type: pathIdType, id: pathId })
  }, [pathId])
  return (
    <div
      id='sidebar-channels'
      className='bg-[#2b2d31] w-[300px] h-full'
    >
      <div className='w-full h-[94%] bg-inherit'>
        {(() => {
          if (state.value.type == null) {
            return <div id='tum ekran'>sıçış error</div>
          } else if (state.value.type === 'dm') {
            return (
              <div className='flex flex-col gap-y-0.5'>
                {channels.isLoading ? (
                  <div className='animate-pulse px-3 py-2 flex flex-row gap-x-2 bg-[#2b2d31] rounded-lg'>

                  </div>
                ) : (
                  <>
                    {channels.data && channels.data.map((dm: Dm) => (
                      <div className='px-3 py-2 flex flex-row gap-x-2 bg-inherit'>
                        {dm.imageUrl && <img src={dm.imageUrl} />}
                      </div>
                    ))}
                  </>
                )}
              </div>
            )
          } else if (state.value.type === 'server') {
            return (
              <div className='flex flex-col gap-y-0.5 p-2 '>
                {channels.data &&
                  channels.data.map((channel: Channel) => (
                    <Link
                      key={channel.id}
                      href={`/channels/${channel.serverId}/${channel.id}`}
                      id='channel'
                      className='items-center px-3 py-2 flex flex-row gap-x-2 bg-inherit hover:bg-white/10 cursor-pointer rounded-lg'
                    >
                      {(() => {
                        switch (channel.type) {
                          case 'TEXT':
                            return (
                              <svg
                                xmlns='http://www.w3.org/2000/svg'
                                width='24'
                                height='24'
                                fill='none'
                                aria-hidden='true'
                                className='size-5 text-white/40'
                                viewBox='0 0 24 24'
                              >
                                <path
                                  fill='currentColor'
                                  fillRule='evenodd'
                                  d='M10.99 3.16A1 1 0 1 0 9 2.84L8.15 8H4a1 1 0 0 0 0 2h3.82l-.67 4H3a1 1 0 1 0 0 2h3.82l-.8 4.84a1 1 0 0 0 1.97.32L8.85 16h4.97l-.8 4.84a1 1 0 0 0 1.97.32l.86-5.16H20a1 1 0 1 0 0-2h-3.82l.67-4H21a1 1 0 1 0 0-2h-3.82l.8-4.84a1 1 0 1 0-1.97-.32L15.15 8h-4.97l.8-4.84ZM14.15 14l.67-4H9.85l-.67 4z'
                                  clipRule='evenodd'
                                />
                              </svg>
                            )
                          case 'RULES':
                            return (
                              <svg
                                xmlns='http://www.w3.org/2000/svg'
                                viewBox='0 0 512 512'
                              >
                                <path d='M318.6 9.4c-12.5-12.5-32.8-12.5-45.3 0l-120 120c-12.5 12.5-12.5 32.8 0 45.3l16 16c12.5 12.5 32.8 12.5 45.3 0l4-4 106.8 106.7-4 4c-12.5 12.5-12.5 32.8 0 45.3l16 16c12.5 12.5 32.8 12.5 45.3 0l120-120c12.5-12.5 12.5-32.8 0-45.3l-16-16c-12.5-12.5-32.8-12.5-45.3 0l-4 4L330.6 74.6l4-4c12.5-12.5 12.5-32.8 0-45.3l-16-16zm-152 288c-12.5-12.5-32.8-12.5-45.3 0l-112 112c-12.5 12.5-12.5 32.8 0 45.3l48 48c12.5 12.5 32.8 12.5 45.3 0l112-112c12.5-12.5 12.5-32.8 0-45.3l-1.4-1.4 58.8-58.7-45.3-45.3-58.7 58.7-1.4-1.4z'></path>
                              </svg>
                            )
                          case 'ANNOUNCEMENTS':
                            return (
                              <svg
                                xmlns='http://www.w3.org/2000/svg'
                                viewBox='0 0 512 512'
                              >
                                <path d='M480 32c0-12.9-7.8-24.6-19.8-29.6S434.5.2 425.3 9.3L381.7 53c-48 48-113.1 75-181 75H64c-35.3 0-64 28.7-64 64v96c0 35.3 28.7 64 64 64v128c0 17.7 14.3 32 32 32h64c17.7 0 32-14.3 32-32V352h8.7c67.9 0 133 27 181 75l43.6 43.6c9.2 9.2 22.9 11.9 34.9 6.9s19.8-16.6 19.8-29.6V300.3c18.6-8.8 32-32.5 32-60.4s-13.4-51.6-32-60.4zm-64 76.7v262.6C357.2 317.8 280.5 288 200.7 288H192v-96h8.7c79.8 0 156.5-29.8 215.3-83.3'></path>
                              </svg>
                            )
                        }
                      })()}
                      <span>{channel.name}</span>
                    </Link>
                  ))}
              </div>
            )
          }
        })()}
      </div>
      <div className='w-full h-[6%] bg-[#232428] flex items-center p-3 relative flex-row gap-x-2'>
        <UserButton />
        <div className='flex flex-col gap-y-0'>
          <span className='text-slate-300'>{user.user?.username}</span>
          <span>tag</span>
        </div>
      </div>
    </div>
  )
}

const Top = ({ user }: { user: UseUserReturn }) => {
  const state = useContext(CurrentChannelContext)
  console.log('top', state.value.type)
  const channelAndServer =
    state.value.type === 'server'
      ? useQuery({
          queryKey: ['get-channel'],
          queryFn: async () => {
            const serverReq = await fetch('http://localhost:9999/get-server', {
              method: 'POST',
              body: JSON.stringify({ id: state.value.id?.split('/')[1] })
            })
            const server = await serverReq.json()

            const channelsReq = await fetch('http://localhost:9999/get-channel',{
              method: 'POST',
              body: JSON.stringify({ serverId: server.id })
            })
            const channels = await channelsReq.json()
            return { currentServer: server, currentChannel: channels }
          },
          enabled: state.value.type === 'server'
        })
      : (state.value.type == 'dm' ? useQuery({
        queryKey: ['get-dms'],
        queryFn: async () => {
          const dmReq = await fetch('http://localhost:9999/get-dm', {
            method: 'POST',
            body: JSON.stringify({ id: state.value.id?.split('/')[0] })
          })
          const dm = await dmReq.json()
          return dm
        }}) : useQuery({
          queryKey: ['home'],
          queryFn: async () => {
            const dmsReq = await fetch('http://localhost:9999/get-dm', {
              method: 'POST',
              body: JSON.stringify({ userId: user.user?.id })
            })
            const dms = dmsReq.json()
            return dms
          }
        }))
  return (
    <div className='w-full h-14 bg-gray-700 flex flex-row'>
      <div
        id='section-1'
        className='flex flex-row items-center p-3 w-[300px] bg-primary h-full shadow-sm shadow-[#1d1e21] relative'
      >
        {state.value.type === null ? (
          <div>sıçış</div>
        ) : state.value.type === 'dm' ? (
          <div className='w-full rounded bg-black/30 h-full cursor-pointer flex items-center justify-start py-1 px-2 text-white/70'>
            Find or start a conversation
          </div>
        ) : (
          <div className='flex items-center flex-row gap-x-3 w-full font-semibold'>
            <svg
              xmlns='http://www.w3.org/2000/svg'
              viewBox='0 0 20 20'
              fill='currentColor'
              className='size-5'
            >
              <path
                fillRule='evenodd'
                d='M9.293 2.293a1 1 0 0 1 1.414 0l7 7A1 1 0 0 1 17 11h-1v6a1 1 0 0 1-1 1h-2a1 1 0 0 1-1-1v-3a1 1 0 0 0-1-1H9a1 1 0 0 0-1 1v3a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1v-6H3a1 1 0 0 1-.707-1.707l7-7Z'
                clipRule='evenodd'
              />
            </svg>
            <span>{channelAndServer.data?.currentServer.name}</span>
          </div>
        )}
      </div>
      <div
        id='section-2'
        className='flex flex-row items-center gap-x-[6px] p-2 w-full bg-secondary h-full relative shadow-sm shadow-[#1d1e21] font-semibold italic'
      >
        <svg
          xmlns='http://www.w3.org/2000/svg'
          width='24'
          height='24'
          fill='none'
          aria-hidden='true'
          className='size-5 text-white/40'
          viewBox='0 0 24 24'
        >
          <path
            fill='currentColor'
            fillRule='evenodd'
            d='M10.99 3.16A1 1 0 1 0 9 2.84L8.15 8H4a1 1 0 0 0 0 2h3.82l-.67 4H3a1 1 0 1 0 0 2h3.82l-.8 4.84a1 1 0 0 0 1.97.32L8.85 16h4.97l-.8 4.84a1 1 0 0 0 1.97.32l.86-5.16H20a1 1 0 1 0 0-2h-3.82l.67-4H21a1 1 0 1 0 0-2h-3.82l.8-4.84a1 1 0 1 0-1.97-.32L15.15 8h-4.97l.8-4.84ZM14.15 14l.67-4H9.85l-.67 4z'
            clipRule='evenodd'
          />
        </svg>
        <span>{channelAndServer.data?.currentChannel[0].name}</span>
      </div>
    </div>
  )
}

const Main = ({ children }: { children: React.ReactNode }) => {
  const state = useContext(CurrentChannelContext)
  return (
    <div
      id='main'
      className='bg-[#313338] w-full h-full'
    >
      {children}
    </div>
  )
}

export default SidebarTopAndMain
